...
public class JavaMail {
    ...
    public JavaMail() {
		...
		InputStream in = JavaMail.class.getResourceAsStream("/config.properties");
		try {
			prop.load(in);
		} catch (IOException e) {
			System.out.println("加载配置文件信息失败了");
			return;
		}
        properties.setProperty("mail.smtp.host", "smtp.163.com");
        properties.setProperty("mail.sender.username",prop.getProperty("email.account"));
        properties.setProperty("mail.sender.password", prop.getProperty("email.password"));
    ...
    public void doSendHtmlEmail(String subject,String sendHtml,String receiveUser) {
        try {
            ...
            InternetAddress from = new InternetAddress(MimeUtility.encodeWord("监控预警邮件")+" <"+sender_username+">");
            ...
            System.out.println("发送邮件出错了:"+e);
        }finally {
            if(transport!=null){
                try {
                    transport.close();
                } catch (MessagingException e) {
                	System.out.println("邮件传输出错了:"+e);
                }
            }
        }
    }
