...
//						alert("系统暂无数据！");
		...
		var createHtml = function() {
			...
			divid.innerHTML = '';
			if(String(conf.isFixed)=="true"){//不固定表头
				cHeadTable(divid);
			}
			cBodyTh(divid);
			cBodytb(divid, returnData);
			if (String(conf.usePage)=="true") {// 是否分页
		...
		var cHeadTable = function(divid) {
			...
			var cn = "";
			if (String(conf.serNumber)=="false") {
				cn = "none";
			}
		...
		var cBodyTh = function(divid) {
			...
			var xy = "hidden";
			if (conf.height == "100%") {
				if(String(conf.isFixed)=="false") {// //不固定表头
					h= "auto";
				}else{
					xy = "auto";
		...
		var fenyeDiv = function(divid, jsonData) {
			...
			lia.innerHTML = 'Total&nbsp;' + totalRecords + '&nbsp;&nbsp;&nbsp;Each page&nbsp;' + conf.pageSize + '&nbsp;&nbsp;&nbsp;Total&nbsp;' + totalPages + '&nbsp;page';
		...
		var pagesIndex = function(pagecode, pageNow, pageCount) {
			/*
			 * var pagecode = CommonUtil._getValueByName(jsonData,conf.pagecode) ==
			 * undefined ? conf.pagecode
			 * :CommonUtil._getValueByName(jsonData,conf.pagecode); var sten =
